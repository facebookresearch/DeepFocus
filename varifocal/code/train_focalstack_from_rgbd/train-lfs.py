...
# Mode: generate focal stack from RGB-D, process each color channel separately
...
if RESTORE_TRAINED_MODEL:
   RESTORE_LOGS_DIR = "../../model/saved_models-lfs/select/" 
...
if MODE=="TRAIN":
   ...
   startSceneIDX = 110 # index of the starting scene for test
...
INPUT_DIM = 2 #intensity, depth
OUTPUT_DIM = N #defocus image
...
with tf.Session() as sess:       
        ...
        for e in range(0, NUM_EPOCHS):
            ...
            for i in range(PERIOD):
                ...
                for b in range(BATCH_SIZE):
                    ...
                    batch_y[b,0:N,:,:] = np.transpose(focal_stack[idxSample,:,:,:].reshape((1,H,W,N)), (0,3,1,2))                      
                    ii = ii + 1
                    ...
                    if numEvalSamples==0:
                       saver.save(sess, LOGS_DIR + "model.ckpt", e)                        
                        ...
                        for j in range(0,numEvalSamples):
                            ...
                            batch_x_eval = np.zeros((1,INPUT_DIM,H,W))
                            batch_y_eval = np.zeros((1,OUTPUT_DIM,H,W))
                            batch_x_eval[0,0,:,:] = cleanRGB[idxs_eval,:,:].reshape((1,1,H,W))
                            batch_x_eval[0,1,:,:] = diopter[idxs_eval,:,:].reshape((1,1,H,W))
                            batch_y_eval[0,0:N,:,:] = np.transpose(focal_stack[idxs_eval,:,:,:].reshape((1,H,W,N)), (0,3,1,2))
    ...
    if MODE == "TEST":    
        ...
        for i in range(0, numTotalSamples//C, 1):
            ...
            for c in range(0, C):
                    batch_x[0,0,:,:] = cleanRGB[i*C+c,:,:].reshape((1,1,H,W))
                    ...
                    recon = sess.run(model_op, feed_dict={X:batch_x_reshape, Y:batch_y[c,:,:,:].reshape((1,OUTPUT_DIM,H,W))})
                    print(time.clock() - time_start)
                    # save
                    recon_fs_rgb[c,:,:,:] = np.transpose(recon.reshape((1,OUTPUT_DIM,H,W)), (0,2,3,1)) #CHWN
            ...
            if EXPORT_RESULT_IMAGES:            
                for n in range(0,N,1):
                    ...
                    for c in range(0,C):
                        ...
                        im_input[:,:,c] = np.clip(batch_y[c,n,:,:].reshape((H,W)), 0, 1)
            ...
            if REPORT_QUALITY:
               ...
               ssims = cal_ssim_focalstack(recon_fs_rgb, np.transpose(batch_y, (0,2,3,1)),crop_width)
               test_SSIMs.append(ssims.eval())
               psnrs = cal_psnr_focalstack(recon_fs_rgb, np.transpose(batch_y, (0,2,3,1)),crop_width)
