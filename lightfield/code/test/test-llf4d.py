...
startSceneIDX = 14 # index of the starting scene for test
endSceneIDX = 14 # index of the last scene
...
REPORT_QUALITY = True # True

# other parameters
imH, imW = 1024, 1024 # size of image
if MODE=="TEST":
...
with tf.Session() as sess:    
    ...
    if MODE == "TEST":
        ...
        numTotalSamples, cleanRGB, diopter = loadData(DATA_PATH, H,W,imH,imW,numViewsYX,idxsScenes_test,C,depthScale, diopterScale, LOAD_RGBD, GLOBAL_SCALE)
            ...
            if EXPORT_RESULT_IMAGES:
                ...
                for viewY in range(numViewsYX[0]):
                   for viewX in range(numViewsYX[1]):
                      ...
                      #-----------------------------------
                      if EXPORT_8BIT_PNG:
                         fn = "%s/gt_lf%03d_im%02d_%02d.png" % (IMGS_DIR,idxsScenes_test[i],viewY,viewX)
                         save_png(im_input, fn)     
                      elif EXPORT_16BIT_TIFF:
                         fn = "%s/gt_lf%03d_im%02d_%02d.tiff" % (IMGS_DIR,idxsScenes_test[i],viewY,viewX)           
