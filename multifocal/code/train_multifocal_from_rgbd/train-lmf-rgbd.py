...
# Mode: multifocal, decomposition from RGB-D, process each color channel separately
...
if RESTORE_TRAINED_MODEL:
   RESTORE_LOGS_DIR = "../../model/saved_models-lmf-rgbd/select/" 
...
INPUT_DIM = 2 # infocus intensity + depth
...
Y = tf.placeholder("float", [None,N,H,W]) # ground truth focal stack
...
labels, predictions = Y[:,0:N,:,:], fs_op 
...
with tf.Session() as sess:    
    ...
    if MODE == "TRAIN":
        ...
        for e in range(0, NUM_EPOCHS):
            ...
            for i in range(PERIOD):
                ...
                batch_y = np.zeros((BATCH_SIZE,N,H,W))
                for b in range(BATCH_SIZE):                    
                    batch_x[b,0,:,:] = cleanRGB[idxs[ii],:,:].reshape((1,1,H,W))
                    batch_x[b,1,:,:] = diopter[idxs[ii],:,:].reshape((1,1,H,W))
                    batch_y[b,0:N,:,:] = np.transpose(focal_stack[idxs[ii],:,:,:].reshape((1,H,W,N)), (0,3,1,2))             
                    ii = ii + 1                         
                ...
                if (i==PERIOD-1) or (i>0 and (i % EVAL_STEP==0)):                  
                    ...
                    if numEvalSamples>0:
                        ...
                        for j in range(0,numEvalSamples-BATCH_SIZE,BATCH_SIZE):
                            ...
                            batch_y_eval = np.zeros((BATCH_SIZE,N,H,W))
                            for be in range(BATCH_SIZE): 
                                batch_x_eval[be,0,:,:] = cleanRGB[idxs_eval[be],:,:].reshape((1,H,W))
                                batch_x_eval[be,1,:,:] = diopter[idxs_eval[be],:,:].reshape((1,H,W))
                                batch_y_eval[be,0:N,:,:] = np.transpose(focal_stack[idxs_eval[be],:,:,:].reshape((1,H,W,N)), (0,3,1,2))  
    ...
    if MODE == "TEST":
        ...
        batch_y = np.zeros((C,N,H,W), dtype=np.float32)
        for i in range(0, numTotalSamples//C, 1):
            v = validSceneIdxList[i]
            for c in range(0, C):
                # generate current batch
                batch_x[c,0,:,:] = cleanRGB[i*C+c,:,:].reshape((1,1,H,W))
                batch_x[c,1,:,:] = diopter[i*C+c,:,:].reshape((1,1,H,W))

                batch_y[c,0:N,:,:] = np.transpose(focal_stack[i*C+c,:,:,:].reshape((1,H,W,N)), (0,3,1,2))   
                ...
                recon_ds, recon_fs = sess.run([model_op, fs_op], feed_dict={X:batch_x_reshape[c,:,:,:].reshape((1,INPUT_DIM*(INTERLEAVE_RATE**2),H//INTERLEAVE_RATE,W//INTERLEAVE_RATE)), Y:batch_y[c,:,:,:].reshape((1,N,H,W))})
            ...
            if EXPORT_RESULT_IMAGES:                  
                for n in range(0,N,1):
                    ...
                    for c in range(0,C):
                        ...
                        im_input[:,:,c] = np.clip(batch_y[c,n,:,:].reshape((H,W))/RENORM_SCALE, 0, 1)
