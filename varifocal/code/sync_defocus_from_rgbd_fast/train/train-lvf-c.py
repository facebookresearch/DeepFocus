        ...
        for e in range(0, NUM_EPOCHS):
            ...
            for i in range(PERIOD):
                ...
                for b in range(BATCH_SIZE):
                    ...
                    batch_x[b,0:C,:,:] = np.transpose(cleanRGB[idxSample,:,:,:].reshape((1,H,W,C)), (0,3,1,2))
                    batch_x[b,C,:,:] = diopter[idxSample,:,:].reshape((1,1,H,W))
                    dm = depthScale * depth[idxSample,:,:].reshape((1,1,H,W)) # remember to convert to original un-scaled depth values
                    batch_x[b,C+1,:,:] = calCoC(D, film_len_dist, dp_focal[idxFocal], fov, H, W, imW, dm, cocScale)
                    batch_y[b,0:C,:,:] = np.transpose(focal_stack[idxSample,:,:,:,idxFocal].reshape((1,H,W,C)), (0,3,1,2))                      
                    ii = ii + 1
                    ...
                    if numEvalSamples==0:
                       saver.save(sess, LOGS_DIR + "model.ckpt", e) 
                        ...
                        for j in range(0,numEvalSamples):
                            ...
                            im = np.transpose(cleanRGB[idxs_eval,:,:,:].reshape((1,H,W,C)), (0,3,1,2)) 
                            dm = depthScale * depth[idxs_eval,:,:].reshape((1,1,H,W)) # remember to convert to original un-scaled depth values
                            for n in range(N):
                                batch_x_eval[n,0:C,:,:] = im
                                batch_x_eval[n,C,:,:] = diopter[idxs_eval,:,:].reshape((1,1,H,W))
                                batch_x_eval[n,C+1,:,:] = calCoC(D, film_len_dist, dp_focal[n], fov, H, W, imW, dm, cocScale)
                                batch_y_eval[n,0:C,:,:] = np.transpose(focal_stack[idxs_eval,:,:,:,n].reshape((1,H,W,C)), (0,3,1,2))     
