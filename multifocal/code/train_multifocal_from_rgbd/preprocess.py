...
def normalize_rgb_exr(im):
    #im = np.power(abs(np.asarray(im)), 1/2.2) # b-g-r #comment out because the exr have been preprocessed
...
def loadData(data_path, RENORM_SCALE, N, M, H, W, imH, imW, dp_focal,dp_display,fov,D,startSceneIDX,endSceneIDX,numEvalScenes,numChannels, depthScale, diopterScale, LOAD_FS,LOAD_RGBD):
    ...
    for s in range(0, numScenes): 
       ...
       if LOAD_RGBD:
           ...
           im = cv2.imread("%s/seed%04d/%d/in_clean_pass.exr" % (data_path, v, imW), -1)             
       ...
       if LOAD_FS:
          for i in range(0, len(dp_focal)):
             im = cv2.imread("%s/seed%04d/%d/in_frame%04d.exr" % (data_path, v, imW, i), -1)             
