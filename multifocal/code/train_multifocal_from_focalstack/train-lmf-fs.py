...
# Mode: multifocal, decomposition from focal stack, process each color channel separately
...
if RESTORE_TRAINED_MODEL:
   RESTORE_LOGS_DIR = "../../model/saved_models-lmf-fs/select/" 
...
INPUT_DIM = N # focal stack
OUTPUT_DIM = M # display stack

# tf Graph input (only pictures)
X = tf.placeholder("float", [None,INPUT_DIM*(INTERLEAVE_RATE**2),H//INTERLEAVE_RATE,W//INTERLEAVE_RATE]) #focal stack
Y = tf.placeholder("float", [None,INPUT_DIM,H,W]) #focal stack
...
def model(x_in):
    ...
    if RENORMALIZE_INPUT:
       x_in = (x_in - 0.5*RENORM_SCALE) # assume the original intensity/depth is [0, 1]
...
# compute focal stack
fs_op = calFSfromDisp_NCHW(model_op, kernel_width)

# define loss and optimizer
labels, predictions = Y, fs_op
...
with tf.Session() as sess:    
    ...
    if MODE == "TRAIN":
        ...
        for e in range(0, NUM_EPOCHS):
            ...
            for i in range(PERIOD):
                ...
                batch_y = np.zeros((BATCH_SIZE,INPUT_DIM,H,W))
                for b in range(BATCH_SIZE):                  
                    batch_x[b,0:N,:,:] = np.transpose(focal_stack[idxs[ii],:,:,:].reshape((1,H,W,N)), (0,3,1,2))                 
                    batch_y[b,0:N,:,:] = batch_x[b,0:N,:,:]
                    ii = ii + 1                         
                ...
                if (i==PERIOD-1) or (i>0 and (i % EVAL_STEP==0)):
                    ...
                    if numEvalSamples==0:
                       saver.save(sess, LOGS_DIR + "model.ckpt", e) 
                        ...
                        ell_psnr_eval_mean = []
                        ell_ssim_eval_mean = []
                        for k in range(0,numEvalSamples-BATCH_SIZE,BATCH_SIZE):
                            idxs_eval = np.linspace(start=i, stop=i+BATCH_SIZE-1, num=BATCH_SIZE, dtype=np.int16) + numTrainSamples  # for valiation use, no randomness
                            batch_x_eval = np.zeros((BATCH_SIZE,INPUT_DIM,H,W))
                            batch_y_eval = np.zeros((BATCH_SIZE,INPUT_DIM,H,W))
                            for be in range(BATCH_SIZE): 
                                batch_x_eval[be,0:N,:,:] = np.transpose(focal_stack[idxs_eval[be],:,:,:].reshape((1,H,W,N)), (0,3,1,2))  
                                batch_y_eval[be,0:N,:,:] = batch_x_eval[be,0:N,:,:]
    ...
    if MODE == "TEST":
        ...
        batch_y = np.zeros((C,INPUT_DIM,H,W), dtype=np.float32)
        for i in range(0, numTotalSamples//C, 1):
            v = validSceneIdxList[i]
            for c in range(0, C):
                # generate current batch
                batch_x[c,0:N,:,:] = np.transpose(focal_stack[i*C+c,:,:,:].reshape((1,H,W,N)), (0,3,1,2)) 
                batch_y[c,0:N,:,:] = batch_x[c,0:N,:,:]
                
                batch_x_reshape = interleave_np(INTERLEAVE_RATE, batch_x)

                # run evauation
                time_start = time.clock()
                recon_ds, recon_fs = sess.run([model_op, fs_op], feed_dict={X:batch_x_reshape[c,:,:,:].reshape((1,INPUT_DIM*(INTERLEAVE_RATE**2),H//INTERLEAVE_RATE,W//INTERLEAVE_RATE)), Y:batch_y[c,:,:,:].reshape((1,INPUT_DIM,H,W))})
                print(time.clock() - time_start)
                
                # save
                recon_ds_rgb[c,:,:,:] = np.transpose(recon_ds.reshape((1,M,H,W)), (0,2,3,1)) 
                recon_fs_rgb[c,:,:,:] = np.transpose(recon_fs.reshape((1,N,H,W)), (0,2,3,1)) 
            ...
            if EXPORT_RESULT_IMAGES:
                for n in range(0,N,1):
                    ...
                    for c in range(0,C):
                        ...
                        im_input[:,:,c] = np.clip(batch_x[c,n,:,:].reshape((H,W))/RENORM_SCALE, 0, 1)
            ...
            if REPORT_QUALITY:
               ...
               psnrs = cal_psnr_focalstack(recon_fs_rgb/RENORM_SCALE, np.transpose(batch_y[:,0:N,:,:],(0,2,3,1))/RENORM_SCALE,crop_width)
               test_PSNRs.append(psnrs)
               ssims = cal_ssim_focalstack(recon_fs_rgb/RENORM_SCALE, np.transpose(batch_y[:,0:N,:,:],(0,2,3,1))/RENORM_SCALE,crop_width)
